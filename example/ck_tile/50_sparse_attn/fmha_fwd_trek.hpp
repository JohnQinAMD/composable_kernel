...
struct fmha_vsa_bwd_args
{
    ...
    // M-major LUT consumed by the dq-only half of the split bwd.
    // Fixed-topk layout: each row has exactly kv_blocks_per_row absolute
    // K-block indices. q_scale = caller_BLKQ / kM0 — when > 1, several
    // CK Q-tiles share one LUT row.
