...
//
// Tier 2.5 Step 1: dKdV-only variant of the VSA bwd pipeline. Fork of
// block_fmha_bwd_dq_dk_dv_pipeline_vsa.hpp with all dQ accumulation stripped
// out. Returned from operator() as (dk_acc, dv_acc). The host dispatcher
// runs this K-major kernel first, then launches a separate Q-major dQ-only
// kernel (similar structure to the fwd VSA pipeline) to produce dQ.
...
struct BlockFmhaBwdDKDVOnlyVSA
{
    ...
    operator()(void* smem_ptr,
               ...
               // VSA sparse iteration: per-K-block transposed LUT of Q-block
               // indices that selected this K-block, plus the per-K-block count.
               // See Stage 7 Tier 2 notes in plan file for construction details.
               const int* kq_lut_ptr,
               int kq_count,
               FmhaMask mask,
               ...
               FmhaDropout& dropout) const
    {
        ...
        // VSA sparse bwd: Q-iteration comes from the transposed LUT instead of
        // a contiguous [seqlen_q_start, seqlen_q_end) range. Each WG owns one
        // K-block (column of the attention pattern); kq_count is the number of
        // Q-blocks that actually selected this K-block, and kq_lut_ptr[0..kq_count)
        // is the list of those Q-block indices in ascending order.
        const auto num_total_loop = amd_wave_read_first_lane(kq_count);
        ...
        const int first_q_block = __builtin_amdgcn_readfirstlane(kq_lut_ptr[0]);
        const index_t seqlen_q_start = first_q_block * kM0;
        // Preserve dense pipeline compatibility: downstream mask.IsEdgeTile
        // checks expect this name; SLA is non-causal so mask is identity.
        const index_t seqlen_q_end = seqlen_q_start + num_total_loop * kM0;
        (void)seqlen_q_end;
        KDataType* k_lds_ptr =
        ...
        // dKdV-only variant: dq_dram_window + QGradBlockTileType unused.
        using SPBlockTileType     = decltype(gemm_0.MakeCBlockTile());
        using SPGradBlockTileType = decltype(gemm_2.MakeCBlockTile());
        (void)dq_dram_block_window_tmp; // unused; kept in signature for ABI compat
        ...
        // Hot loop (VSA sparse Q-iteration)
        while(i_total_loops < num_total_loop)
        {
            // VSA: compute jump from current Q-block to the next one in the
            // transposed LUT. On the last iter the jump is harmless because the
            // window won't be dereferenced again.
            const int cur_q_block =
                __builtin_amdgcn_readfirstlane(kq_lut_ptr[i_total_loops]);
            const int next_q_block = __builtin_amdgcn_readfirstlane(
                (i_total_loops + 1 < num_total_loop) ? kq_lut_ptr[i_total_loops + 1]
                                                     : (cur_q_block + 1));
            const index_t q_jump = (next_q_block - cur_q_block) * kM0;

            auto q_block_tile = load_tile(q_dram_window);
            move_tile_window(q_dram_window, {q_jump, 0});

            auto lse_block_tile = load_tile(lse_dram_window);
            move_tile_window(lse_dram_window, {q_jump});
            ...
            // P1.4: consume log2-space LSE directly (wrapper passes it
            // through without the natural-log divide). Drops the per-row
            // log2e multiply from the hot loop.
            auto p                 = SPBlockTileType{};
            constexpr auto p_spans = decltype(p)::get_distributed_spans();
            sweep_tile_span(p_spans[number<0>{}], [&](auto idx0) {
                constexpr auto i_idx = make_tuple(idx0);
                auto row_lse         = get_validated_lse(lse[i_idx]);
            ...
            move_tile_window(do_dram_window, {q_jump, 0});

            auto d_block_tile = load_tile(d_dram_window);
            move_tile_window(d_dram_window, {q_jump});
            ...
            // Tier 2.5 Step 1: dKdV-only variant. Everything below this line
            // in the original pipeline (ds_lds store, Gemm4 dq_acc, dq store,
            // dq window advance) has been stripped. The only per-iter work
            // left is advancing the shared Q/dO/LSE/D windows.
            (void)qt_reg_tensor; // silence unused if compiler misses gemm_3
            (void)kt_reg_tensor; // only used by the dropped Gemm4

            i_total_loops += 1;
            seqlen_q_step += q_jump;
        }
