...
//
// dK/dV-only variant of the VSA bwd pipeline (K-major). operator()
// returns (dk_acc, dv_acc); dq accumulation is performed by the
// companion dq-only pipeline in a separate kernel launch. Host dispatch
// is in vsa_sparse_attention_bwd.cpp.
...
struct BlockFmhaBwdDKDVOnlyVSA
{
    ...
    operator()(void* smem_ptr,
               ...
               // VSA sparse iteration: per-K-block transposed LUT of
               // Q-block indices that selected this K-block, plus the
               // per-K-block valid count.
               ...
               FmhaDropout& dropout) const
    {
        ...
        while(i_total_loops < num_total_loop)
        {
            ...
            // dKdV-only: the dq/dbias half of the pipeline (ds_lds
            // store, Gemm4 dq_acc, dq store, dq window advance) is
            // handled by the companion dq-only pipeline. All that's
            // left per iter is advancing the Q/dO/LSE/D windows.
